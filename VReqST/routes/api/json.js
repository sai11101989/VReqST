...
Router.get("/:json_id/scene", authenticate, async (req, res) => {
  try {
...
Router.get("/:json_id/action", authenticate, async (req, res) => {
  try {
    const owner = req?.user.id;
    const jsonfiles = await Json.find({
      $or: [{ private: false }, { ownerid: owner }],
...
Router.get("/:json_id/asset", authenticate, async (req, res) => {
  try {
    const owner = req?.user.id;
    const jsonfiles = await Json.find({
      $or: [{ private: false }, { ownerid: owner }],
...
Router.get("/:json_id/custom", authenticate, async (req, res) => {
  try {
    const owner = req?.user.id;
    const jsonfiles = await Json.find({
      $or: [{ private: false }, { ownerid: owner }],
      $and: [{ grammarfor: "custom" }],
    })
      .populate("ownerid")
      .exec();
    res.status(200).json(jsonfiles);
...
Router.patch("/:json_id/asset", authenticate, async (req, res) => {
  try {
    const jsonfile = await Json.findById(req.params.json_id);
    const ownerid = req?.user.id;
    ...
    const { name, data, grammarfor } = req.body;

    const updatejson = await Json.findByIdAndUpdate(
      req.params.json_id,
      { name, asset:data, ownerid, grammarfor },
      { new: true }
    );
...
Router.patch("/:json_id/action", authenticate, async (req, res) => {
  try {
    const jsonfile = await Json.findById(req.params.json_id);
    const ownerid = req?.user.id;
    if (jsonfile.ownerid.toString() !== ownerid) {
    ...
    const updatejson = await Json.findByIdAndUpdate(
      req.params.json_id,
      { name, action:data, ownerid, grammarfor },
      { new: true }
    );
...
Router.patch("/:json_id/custom", authenticate, async (req, res) => {
  try {
    const jsonfile = await Json.findById(req.params.json_id);
    const ownerid = req?.user.id;
    if (jsonfile.ownerid.toString() !== ownerid) {
    ...
    const updatejson = await Json.findByIdAndUpdate(
      req.params.json_id,
      { name, custom:data, ownerid, grammarfor },
      { new: true }
    );
...
Router.patch("/:json_id/timeline", authenticate, async (req, res) => {
  try {
    const jsonfile = await Json.findById(req.params.json_id);
    const ownerid = req?.user.id;
    if (jsonfile.ownerid.toString() !== ownerid) {
    ...
    const updatejson = await Json.findByIdAndUpdate(
      req.params.json_id,
      { name, timeline:data, ownerid, grammarfor },
      { new: true }
    );
...
Router.patch("/:json_id/scene", authenticate, async (req, res) => {
  try {
    const jsonfile = await Json.findById(req.params.json_id);
    const ownerid = req?.user.id;
    if (jsonfile.ownerid.toString() !== ownerid) {
    ...
    const updatejson = await Json.findByIdAndUpdate(
      req.params.json_id,
      { name, scene:data, ownerid, grammarfor },
      { new: true }
    );
