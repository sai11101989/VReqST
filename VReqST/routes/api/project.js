...
Router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { name,grammarName } = req.body;
...
Router.patch("/:projectid/scene", authenticate, async (req, res) => {
  try {
    ...
    console.log(project);
    if (project.action.length > 0 && project.asset.length > 0&&  project.custom.length > 0&& project.timeline.length > 0) {
...
Router.get("/:projectid/grammarName", authenticate, async (req, res) => {
  try {
    const project = await Project.findById(req.params.projectid).select({
      asset: 0,
      action: 0,
...
Router.patch("/:projectid/asset", authenticate, async (req, res) => {
  try {
    ...
    console.log(project);
    if (project.action.length > 0&& project.scene.length > 0&& project.custom.length > 0&& project.timeline.length > 0) {
      await Project.findByIdAndUpdate(req.params.projectid, {
        asset: req.body.data,
        step: 2,
        isFinished: true,
...
Router.patch("/:projectid/action", authenticate, async (req, res) => {
  try {
    ...
    if ( project.asset.length > 0&& project.scene.length > 0&& project.custom.length > 0&& project.timeline.length > 0) {
      await Project.findByIdAndUpdate(req.params.projectid, {
...
Router.patch("/:projectid/custom", authenticate, async (req, res) => {
  try {
    const project = await Project.findById(req.params.projectid);
    if (!project) {
      return res.status(400).send({ message: "Project not found" });
    ...
    if (project.action.length > 0 && project.asset.length > 0&& project.scene.length > 0&&  project.timeline.length > 0) {
      await Project.findByIdAndUpdate(req.params.projectid, {
        custom: req.body.data,
        step: 3,
        isFinished: true,
      });
    } else {
...
Router.get("/:projectid/custom", authenticate, async (req, res) => {
  try {
    const project = await Project.findById(req.params.projectid).select({
      asset: 0,
      scene: 0,
...
Router.patch("/:projectid/timeline", authenticate, async (req, res) => {
  try {
    const project = await Project.findById(req.params.projectid);
    if (!project) {
      return res.status(400).send({ message: "Project not found" });
    ...
    if (project.action.length > 0 && project.asset.length > 0&& project.scene.length > 0&& project.custom.length > 0) {
      await Project.findByIdAndUpdate(req.params.projectid, {
...
Router.get("/:projectid/timeline", authenticate, async (req, res) => {
  try {
    const project = await Project.findById(req.params.projectid).select({
      asset: 0,
      scene: 0,
